...
public class AverageCost {
	public static class AverageCostMapper extends
	Mapper<LongWritable, Text, Text,Text > 
	{
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException 
		{
			...
			String finalvalue=record[7]+","+record[6];
			context.write(finalkey, new Text(finalvalue));
		}
	}
	public static class AverageCostReducer extends
	...
	private TreeMap <DoubleWritable, Text> AvgMap = new TreeMap<DoubleWritable, Text>();
		public void reduce(Text key, Iterable <Text> value, Context context) throws IOException, InterruptedException
		{
			...
			String answer= (key.toString()+","+average+","+SUMtotalcost+","+SUMtotalqty);
...
//				AvgMap.remove(AvgMap.firstKey());
	...
	public static void main(String args[]) throws  IOException, ClassNotFoundException, InterruptedException
	{
		...
		Job job = Job.getInstance(conf, "Cost/Quantity");
	    job.setJarByClass(AverageCost.class);
	    job.setMapperClass(AverageCostMapper.class);
	    job.setReducerClass(AverageCostReducer.class);
